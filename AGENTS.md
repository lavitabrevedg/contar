...
- Unity 실행 상태가 작업 안정성이나 검증 정확도에 영향을 주는 경우, 작업을 시작하기 전에 적합한 상태를 판단해 사용자에게 먼저 안내한다.
- 코드 컴파일, 자동 테스트, 배치 빌드, 정량 프로파일링처럼 Unity를 종료한 상태가 유리한 작업은 사용자에게 Unity를 꺼 달라고 요청한 뒤 진행한다.
- 씬·프리팹·Inspector 편집, Game View 확인, Play Mode 조작처럼 Unity 실행이 필요한 작업은 사용자에게 Unity를 켜 두거나 열어 달라고 요청한 뒤 진행한다.
- Unity를 종료하도록 요청할 때는 미저장 씬을 먼저 저장하고 Play Mode를 종료하도록 함께 안내한다.
- Unity 실행 여부가 작업에 영향을 주지 않는 경우에는 불필요하게 상태 변경을 요청하지 않는다.
